import sys
sys.path.append('/home/syl/DevInSight/DevNet/development')

import math
from so import so_uniform_init_
from expm import expm, rescaled_matrix_exp
import torch
...
class development_layer(nn.Module):
    ...
    def forward(self, 
        ...
        Args:
            ...
            mask (torch.tensor): mask tensor with shape (N,T). Defaults to None
            kernel (int): kernel size for the convolutional layer. Defaults to None
        ...
        if self.return_sequence:
            ...
            #     M_dX = self.projection(dX1).reshape(
            #         N, -1, self.channels, self.hidden_size, self.hidden_size)
            #     # # fill padded area with identity matrix I
            #     # if mask is not None:
            #     #     I = torch.eye(self.hidden_size, device=input.device, dtype=input.dtype).reshape(
            #     #         1, self.hidden_size, self.hidden_size).repeat(self.channels, 1, 1)
            #     #     mask_diff = mask[:, 1+i:i+time_len] * mask[:, i:i+time_len-1]
            #     #     M_dX[mask_diff == 0] = I
            #     out = torch.einsum('bcij,bcjk->bcik', out,
            #                        self.dyadic_prod(M_dX))
            
            dX1 = dX.reshape(-1, dX.shape[-1])
            M_dX = self.projection(dX1).reshape(
                N, -1, self.channels, self.hidden_size, self.hidden_size)
            # fill padded area with identity matrix I
            if mask is not None:
            ...
            if kernel is not None: 
                stride = stride if stride is not None else kernel//2 
                M_dX = M_dX.unfold(1, kernel-1, stride)    # (N,T',c,m,m,kernel-1)
                M_dX = M_dX.reshape(
                    -1, self.channels, self.hidden_size, self.hidden_size, kernel-1
                    ).permute(0, 4, 1, 2, 3)  # (N*T',kernel-1,c,m,m)
                out = torch.eye(
                    self.hidden_size, device=input.device, dtype=input.dtype
                    ).reshape(
                        1, 1, self.hidden_size, self.hidden_size
                        ).repeat(M_dX.shape[0], self.channels, 1, 1)
                out = torch.einsum('bcij,bcjk->bcik', out,
                                    self.dyadic_prod(M_dX))
                out = out.reshape(N, -1, self.channels, self.hidden_size, self.hidden_size)
            else:
                out = torch.einsum('bcij,bcjk->bcik', out,
                                    self.dyadic_prod(M_dX))
    ...
    def dyadic_prod(X: torch.tensor) -> torch.tensor:
        ...
        Args:
            X (torch.tensor): A batch of matrix time series, shape (N,T,C,m,m)

        Returns:
            torch.tensor: cumulative product on time dimension, shape (N,C,m,m)
        """
        N, T, C, m, m = X.shape
