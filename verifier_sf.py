...
if __name__ == "__main__":
    ...
    for k in numpy.arange(N_user): 
        ...
        idxTest = list(set(range(len(ind))) - set(idxTemp))
        test = [dset[i] for i in idxTest]
    ...
    numpy.save(
        f"DET/{args.index}/{args.dataset}_sf_4vs1.npy",
        numpy.array([DET_FAR["4vs1"], DET_FRR["4vs1"], N_g, N_n], dtype=object), 
        allow_pickle=True,
    )
    numpy.save(
        f"DET/{args.index}/{args.dataset}_sf_1vs1.npy",
    ...
    with open(filename, "r+") as f:
        ...
        constuct(results, [index, dataset, "sf"])
        results[index][dataset]["sf"] = EERs
